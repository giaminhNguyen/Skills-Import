...
def export_flow_txt(project: Path, output: Path, chapters: list[tuple[int, Path, str]], meta: dict) -> None:
    """Single continuous file: no title, no chapter/volume headings, no dividers.

    Intended for TTS/audio pipelines (e.g. gen-audio-queue) and for readers who
    want one uninterrupted flow. Chapter boundaries are not written to the file
    at all — the writer still plans/drafts per-chapter internally, but nothing
    reader-facing marks where one chapter ends and the next begins.
    """
    chunks = []
...
def main() -> None:
    ...
    p.add_argument("--flow", action="store_true",
                    help="TXT only: single continuous file, no title/chapter/volume headings or dividers "
                         "(for TTS/audio pipelines or uninterrupted reading).")
    args = p.parse_args()
    ...
    if args.output:
        ...
        suffix = "-flow.txt" if args.flow else ".txt"
        output = project / f"{slugify(meta['title']) or 'novel'}{suffix}"
    ext = output.suffix.lower()
    if ext not in {".txt", ".epub"}:
        output = output.with_suffix(".txt")
        ext = ".txt"
    if args.flow and ext != ".txt":
        raise SystemExit("--flow only applies to .txt output")
    ...
    if ext == ".txt":
        if args.flow:
            export_flow_txt(project, output, chapters, meta)
        else:
