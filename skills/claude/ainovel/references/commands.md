...
Pass `--flow` (TXT only) for a single continuous file with no title, no chapter/volume headings, and no `---` dividers — one unbroken stream of prose from the first committed chapter to the last. The writer still plans and drafts per chapter internally (contracts, consistency, review all stay chapter-scoped); `--flow` only affects what the reader-facing export shows. Default output name gets a `-flow` suffix so it never collides with the normal TXT export. Use this whenever the export feeds a downstream tool that treats chapter breaks as noise — e.g. the `gen-audio-queue` skill, which turns a `.txt` into narrated audio and has no use for headings.
