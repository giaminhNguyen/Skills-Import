...
(function() {
  ...
  const gradientTransformToHandlePositions = (transform) => {
    if (!transform || !Array.isArray(transform) || transform.length < 2) return void 0;
  ...
  const serializePaintsTyped = (paints) => {
    if (isMixed(paints)) return void 0;
    if (!paints || !Array.isArray(paints)) return void 0;
  ...
  const serializeEffects = (effects) => {
    if (isMixed(effects)) return void 0;
    if (!effects || !Array.isArray(effects)) return void 0;
  ...
  const serializeConstraints = (node) => {
    if (!("constraints" in node) || !node.constraints) return void 0;
    return {
      horizontal: node.constraints.horizontal,
      vertical: node.constraints.vertical
    };
  };
  const serializeAbsoluteBoundingBox = (node) => {
  ...
  const serializeAutoLayoutProperties = (node) => {
    const result = {};
  ...
  const serializeStrokeProperties = (node) => {
    const result = {};
  ...
  const serializeCornerRadii = (node) => {
  ...
  const resolveLineHeightPx = (node) => {
    if (isMixed(node.lineHeight)) return void 0;
    const lh = node.lineHeight;
    if (!lh || lh.unit === "AUTO") return void 0;
  ...
  const resolveLetterSpacingPx = (node) => {
  ...
  const serializeTextStyle = (node) => {
    var _a, _b;
    const style = {};
    if (!isMixed(node.fontName) && node.fontName) {
      style.fontFamily = node.fontName.family;
      style.fontPostScriptName = node.fontName.style ? `${node.fontName.family}-${node.fontName.style.replace(/\s+/g, "")}` : void 0;
    } else if (isMixed(node.fontName)) {
      style.fontFamily = "mixed";
    }
    style.fontWeight = isMixed(node.fontWeight) ? 400 : (_a = node.fontWeight) != null ? _a : 400;
    style.fontSize = isMixed(node.fontSize) ? 12 : (_b = node.fontSize) != null ? _b : 12;
  ...
  const deduplicateStyles = (tree) => {
    ...
    const countWalk = (node) => {
      ...
      if (Array.isArray(node.fills)) counts.set("f:" + JSON.stringify(node.fills), ((_c = counts.get("f:" + JSON.stringify(node.fills))) != null ? _c : 0) + 1);
      if (Array.isArray(node.strokes)) counts.set("s:" + JSON.stringify(node.strokes), ((_d = counts.get("s:" + JSON.stringify(node.strokes))) != null ? _d : 0) + 1);
    ...
    const replaceWalk = (node) => {
      ...
      if (Array.isArray(node.fills)) {
        ...
        if (ref) result = __spreadProps(__spreadValues({}, result), { fills: ref });
      }
      if (Array.isArray(node.strokes)) {
        const ref = keyToRef.get("s:" + JSON.stringify(node.strokes));
        if (ref) result = __spreadProps(__spreadValues({}, result), { strokes: ref });
      }
